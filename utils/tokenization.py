...
def tokenize_cn(text: str, min_len: int = 0, only_cnchr: bool=False) -> list[list[str]]:
    """
    ...
    for sentence in sentences:
        ...
        if only_cnchr:
            tokens = [NON_CN_PATTERN.sub('', token) for token in tokens]
...
def tokenize_en(text: str, min_len: int = 0, only_alpha: bool=False) -> list[list[str]]:
    """
    ...
    for sentence in sentences:
        ...
        if only_alpha:
            tokens = [NON_EN_PATTERN.sub('', token) for token in tokens]
        tokens = [token for token in tokens if token]
        if len(tokens) >= min_len:
            tokenized_sentences.append(tokens)
